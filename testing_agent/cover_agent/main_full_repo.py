...
import copy
import os
from testing_agent.cover_agent.AICaller import AICaller
from testing_agent.cover_agent.utils import parse_args_full_repo, find_test_files
from testing_agent.cover_agent.CoverAgent import CoverAgent
from testing_agent.cover_agent.lsp_logic.ContextHelper import ContextHelper
from testing_agent.cover_agent.lsp_logic.utils.utils_context import expand_source_context
...
async def run():
    ...
    async with context_helper.start_server():
        ...
        for test_file in test_files:
            ...
            #{
            if source_file and context_helper._lsp:
                extra_paths = await expand_source_context(source_file, context_helper, args)
                context_files_include.extend(p for p in extra_paths
                                            if Path(p).resolve() != Path(source_file).resolve())
                context_files_include = list(dict.fromkeys(context_files_include))
            #}
