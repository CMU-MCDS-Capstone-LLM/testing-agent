...
from testing_agent.cover_agent.lsp_logic.file_map.file_map import FileMap
from testing_agent.cover_agent.lsp_logic.multilspy import LanguageServer
from testing_agent.cover_agent.lsp_logic.multilspy.multilspy_config import MultilspyConfig
from testing_agent.cover_agent.lsp_logic.multilspy.multilspy_logger import MultilspyLogger
...
#{
async def expand_source_context(source_file, context_helper, args):
    rel_source = os.path.relpath(source_file, args.project_root)
    # reuse FileMap to get captures for the source file itself
    fname_summary = FileMap(
        os.path.join(args.project_root, rel_source),
        parent_context=False, child_context=False, header_max=0,
        project_base_path=args.project_root,
    )
    _, captures = fname_summary.get_query_results()
    lsp = context_helper._lsp
    direct_files, _ = await lsp.get_direct_context(
        captures, args.project_language, args.project_root, rel_source
    )
    reverse_files, _ = await lsp.get_reverse_context(
        captures, args.project_root, rel_source
    )
    return list({*direct_files, *reverse_files})
#}
